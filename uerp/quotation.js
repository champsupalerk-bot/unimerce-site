/* ==========================================================================
   ...
   ใช้ข้อมูลจาก UERP Global Cache
   - Customer: window.UERP_CACHE.customers
   - Product:  window.UERP_CACHE.products
   หมายเหตุ:
   การโหลด/Refresh Cache จัดการโดย uerp-cache.js
   ========================================================================== */
let cart = [];
let customerMode = 'db';
// ----------------------------------------------------
...
function getCachedCustomers() {
    return window.UERP_CACHE?.customers || [];
}
function getCachedProducts() {
    return window.UERP_CACHE?.products || [];
}
// รอให้ Global Cache โหลดเสร็จ
async function waitForCache() {
    if (window.UERP_CACHE_READY) {
        await window.UERP_CACHE_READY;
    }
    return {
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        await waitForCache();
        console.log(
            `UERP Quotation: Cache Ready | Customers: ${getCachedCustomers().length} | Products: ${getCachedProducts().length}`
        );
    } catch (err) {
        console.error(
            'UERP Quotation: Cache unavailable',
            err
        );
    } finally {
...
function showSearchLoading(isLoading) {
    ...
    if (isLoading) {
        ...
        if (prodInput) {
            prodInput.placeholder =
                "พิมพ์รหัสสินค้า (Item_Code) หรือ รายละเอียด (Description)...";
        }
    }
}
// ----------------------------------------------------
...
function selectCustomer(cust) {
    ...
    if (custName) {
        custName.value = cust.customer_name || '';
    }
    if (custTaxId) {
        custTaxId.value = cust.tax_id || '';
    }
    if (custPhone) {
        custPhone.value = cust.phone || '';
    }
    if (custAddress) {
        custAddress.value = cust.customer_address || '';
    }
    if (custEmail) {
                    ...
                    <td class="py-3 px-2 text-center">
                        ...
                        >
                            ...
                            <option
                                ...
                                ${item.selectedPriceTier === 'PromotionPrice' ? 'selected' : ''}
                            >
                                Promo Price
                            </option>
                            <option
...
function openPreviewModal() {
    ...
    document.getElementById('lblCustName').innerText =
        custName;
    document.getElementById('lblCustAddress').innerText =
        document.getElementById('custAddress').value
            ? 'ที่อยู่: ' +
              document.getElementById('custAddress').value
            : '';
    document.getElementById('lblCustTax').innerText =
        document.getElementById('custTaxId').value
            ? 'เลขประจำตัวผู้เสียภาษี: ' +
              document.getElementById('custTaxId').value
            : '';
    document.getElementById('lblCustPhone').innerText =
        document.getElementById('custPhone').value
            ? 'เบอร์โทร: ' +
              document.getElementById('custPhone').value
            : '';
    document.getElementById('lblCustEmail').innerText =
        document.getElementById('custEmail').value
            ? 'อีเมล: ' +
              document.getElementById('custEmail').value
            : '';
    document.getElementById('lblDocRemark').innerText =
        document.getElementById('docRemark').value || '-';
    const tbody =
        document.getElementById(
            'lblTableBody'
        );
