...
const supabaseKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJ1ZXIiLCJpYXQiOjE3ODE2NTY5NTAsImV4cCI6MjA5NzIzMjk1MH0.Qcq5h2TignXwhsyOe8IYcMYvlayyTjH66tTiPznVOOY";
...
async function fetchTransactions(isManualRefresh = false) {
    ...
    if (
        ...
        cachedData &&
        cachedTime &&
        (nowTime - Number(cachedTime) < CACHE_EXPIRY_MS)
    ) {
        allTransactions = JSON.parse(cachedData);

        const remainingMinutes = Math.round(
            (CACHE_EXPIRY_MS - (nowTime - Number(cachedTime))) / 60000
        );

        updateExecutionLog(
            `Cached: ${allTransactions.length.toLocaleString()} rows (${remainingMinutes}m remaining)`
        );
    ...
    try {
        ...
        updateExecutionLog(
            `Loaded: ${allTransactions.length.toLocaleString()} rows from DB`
        );
        ...
        updateStatus(
            `เกิดข้อผิดพลาดในการโหลด: ${err.message}`,
            true
        );
...
function updateExecutionLog(msg) {
    ...
    if (logEl) {
        logEl.innerHTML =
            `<i class="fa-solid fa-database text-google-blue"></i> ${msg}`;
    }
}
...
function applyFilters() {
    ...
    if (period === 'TODAY') {
        ...
        if (s) {
            startFilterDate = new Date(s);
        }

        if (e) {
            endFilterDate = new Date(e + 'T23:59:59');
        }
    }
...
function renderTable() {
    ...
    if (pageData.length === 0) {
        tbody.innerHTML = `
            <tr>
                <td colspan="7" class="p-6 text-center text-google-gray">
        ...
        pageData.forEach(row => {
            ...
            tr.innerHTML = `
                ...
                <td class="p-2 text-center w-28">
                    <div
                        class="font-bold text-slate-900 font-mono text-xs truncate max-w-[110px]"
                    ...
                    <div
                        class="text-[10px] text-google-gray font-mono truncate max-w-[110px]"
                        title="${row.order_no || '-'}"
                    >
                ...
                <!-- Item Code -->
                <td
                    class="p-2 text-center font-mono font-bold text-google-blue max-w-[95px] truncate"
                    title="${row.item_code || '-'}"
                >
                    ${row.item_code || '-'}

                    <div
                        class="sm:hidden text-[10px] font-normal text-slate-500 truncate max-w-[95px]"
                        title="${row.item_name || ''}"
                    >
                        ${row.item_name || ''}
                    </div>
                ...
                <!-- Item Name -->
                <td
                    class="p-2 text-left font-medium text-slate-800 max-w-[160px] sm:max-w-[240px] truncate"
                    title="${row.item_name || '-'}"
                >
                    ${row.item_name || '-'}
                </td>
                ...
                <td class="p-2 text-center hidden sm:table-cell">
                    <span
                        class="px-1.5 py-0.5 rounded text-[10px] font-bold ${channelBadgeClass}"
                    >
                        ${row.channel || 'Direct'}
                    </span>
                </td>
    ...
    const renderedCount =
        document.getElementById('renderedCount');

    if (renderedCount) {
        renderedCount.innerText =
...
function handleStatusChange(rowId, newStatus) {
    const normalizedStatus =
        String(newStatus || 'PENDING').toUpperCase();

    pendingStatusChanges[rowId] =
        normalizedStatus;

    // Change color immediately without re-rendering table
    const selects =
...
function updatePendingChangesCount() {
    ...
    const countEl =
        document.getElementById('pendingChangesCount');
...
async function submitChanges() {
    ...
    try {
        const updatePromises =
            Object.keys(pendingStatusChanges).map(rowId => {
                const item =
...
function setupInfiniteScroll() {
    const container =
        document.querySelector('.sticky-table-container');
