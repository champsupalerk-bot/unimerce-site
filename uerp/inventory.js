...
fetch('uerpheader.html')
  ...
  .then(data => {
    const placeholder = document.getElementById("headerContainer");
...
const CACHE_KEY = "uerp_products_cache_data";
const CACHE_TIME_KEY = "uerp_products_cache_timestamp";
...
async function fetchAllFromSupabaseInBatches() {
  ...
  while (fetchMore) {
    ...
    updateLog(`Fetching ${batchIndex}...`);

    const pageData = await window.supabaseFetch('products', {
      limit: BATCH_SIZE,
      offset: offset
    });
...
async function loadTable(forceRefresh = false) {
  ...
  try {
    ...
    const isCacheValid =
      cachedTime &&
      cachedData &&
      (now - parseInt(cachedTime, 10) < ONE_DAY_MS);
...
function forceRefreshData() {
  ...
  const icon =
    document.querySelector("#btnRefresh i");
...
function applyAll() {
  ...
  const hideZero =
    document.getElementById("hideZero").checked;

  const showOnlySelected =
    document.getElementById("showOnlySelected").checked;

  const currentCol3 =
    document.getElementById("column3Select").value;
  ...
  viewData =
    [...filteredData].sort((a, b) => {
      ...
      if (
        ...
        let n1 = cleanNumber(v1);
        let n2 = cleanNumber(v2);
...
function renderTable(data) {
  ...
  tbody.innerHTML =
    data.map(p => {
      ...
      if (
        currentCol3 === "ReorderSTATUS" &&
        String(cellValue).trim() === "ORDER!"
      ) {
      ...
      if (
        (
          currentCol3 === "OnHand" ||
          currentCol3 === "PromotionPrice" ||
          currentCol3 === "จำนวนต่อลัง"
        ) &&
        cellValue !== "-"
      ) {

        let num =
          cleanNumber(cellValue);
      ...
      const isSelected =
        selectedItemCodes.has(itemCode);
          ...
          <td class="col-code">
            ...
            <button
              ...
              <svg xmlns="http://www.w3.org/2000/svg"
                ...
                <path d="M 2,7 V 2 H 7"
                      stroke-width="1.5" />

                <path d="M 17,2 H 22 V 7"
                      stroke-width="1.5" />

                <path d="M 2,17 V 22 H 7"
                      stroke-width="1.5" />

                <path d="M 17,22 H 22 V 17"
                      stroke-width="1.5" />
...
function copyRowText(rowElement) {
...
window.addEventListener(
  ...
  () => {
    ...
    document
      .getElementById("showOnlySelected")
      .addEventListener(
        "change",
        () => {
          applyAll();
