...
const supabaseKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmxlIiwidHMiOiJ4eWdkbXN6ZXJubWlyY21icndrZSIsInJvbGUiOiJhbm9uIiwiaWF0IjoxNzgxNjU2OTUwLCJleHAiOjIwOTcyMzI5NTB9.Qcq5h2TignXwhsyOe8IYcMYvlayyTjH66tTiPznVOOY";
...
function applyFilters() {
    ...
    allTransactions.forEach(item => {
        ...
        if (item.order_no) {
            orderCounts[item.order_no] = (orderCounts[item.order_no] || 0) + 1;
        }
    });
...
function renderTable() {
    ...
    if (pageData.length === 0) {
        ...
        pageData.forEach(row => {
            ...
            if (isDup) {
                rowBgClass = "row-duplicate";
            }
            if (isSelected) {
                rowBgClass = "row-selected";
            }
            ...
            tr.innerHTML = `
                ...
                <td class="p-2 text-center hidden sm:table-cell">
                    <select onchange="handleStatusChange('${row._rowId}', this.value)" class="status-select ${statusClass} input-mobile-safe">
...
function handleStatusChange(rowId, newStatus) {
    ...
    updatePendingChangesCount();
    const select = document.querySelector(`select[onchange="handleStatusChange('${rowId}', this.value)"]`);
    if (!select) return;
    select.classList.remove('status-active', 'status-pending', 'status-void');
    if (newStatus === 'ACTIVE') {
        select.classList.add('status-active');
    } else if (newStatus === 'VOID') {
        select.classList.add('status-void');
    } else {
