...
function setPeriod(period, shouldApply = true) {
    ...
    document.getElementById('customDateContainer').classList.toggle('hidden', period !== 'CUSTOM');
...
function handleCustomPeriodChange() {
    ...
    document.querySelectorAll('.period-btn').forEach(btn => {
        btn.classList.remove('bg-google-blue', 'text-white', 'border-google-blue');
        btn.classList.add('bg-white', 'text-slate-600', 'border-google-border');
    });
...
function getUniqueOrderGroups(rows) {
    ...
    rows.forEach(row => {
        const orderNo = String(row.order_no || row.invoice_no || row._rowId);
...
function applyFilters() {
    ...
    if (period === 'TODAY') {
        ...
        if (s) startFilterDate = new Date(s + 'T00:00:00');
    ...
    filteredTransactions = allTransactions.filter(item => {
        if (showPKOnly && !selectedRowIds.has(item._rowId)) {
            return false;
        }
        ...
        const channelMatch =
            channel === 'ALL' ||
            itemChannel.toLowerCase() === channel.toLowerCase();

        const statusMatch =
            status === 'ALL' ||
            itemStatus.toUpperCase() === status.toUpperCase();
        ...
        if (startFilterDate || endFilterDate) {
            ...
            if (startFilterDate && rowDate < startFilterDate) {
                dateMatch = false;
            }

            if (endFilterDate && rowDate > endFilterDate) {
                dateMatch = false;
            }
        }
    ...
    filteredTransactions.sort((a, b) => {
        ...
        if (currentSortColumn === 'sales_amt') {
            valA = Number(valA) || 0;
            valB = Number(valB) || 0;
        } else {
    ...
    filteredOrderGroups = getUniqueOrderGroups(filteredTransactions);
...
function renderTable() {
    ...
    const totalOrders = filteredOrderGroups.length;
    const visibleGroups = filteredOrderGroups.slice(0, displayLimit);
    const pageData = visibleGroups.flatMap(group => group.items);
    ...
    if (pageData.length === 0) {
        ...
        pageData.forEach(row => {
            ...
            if (ch.includes('shopee')) {
                channelBadgeClass = "bg-shopee text-white";
            } else if (ch.includes('lazada')) {
                channelBadgeClass = "bg-lazada text-white";
            } else if (ch.includes('tiktok')) {
                channelBadgeClass = "bg-tiktok text-white";
            }
            ...
            const orderPrefix =
                orderNo.length > 3 ? orderNo.slice(0, -3) : '';

            const orderTail =
                orderNo.length > 3 ? orderNo.slice(-3) : orderNo;
            ...
            tr.innerHTML = `
                <td class="p-2 align-top font-mono text-[11px] text-slate-600 whitespace-nowrap">
    ...
    document.getElementById('orderCountTop').innerText =
...
async function submitChanges() {
    ...
    try {
        ...
        Object.keys(pendingStatusChanges).forEach((rowId, index) => {
            const item = allTransactions.find(t => t._rowId === rowId);

            if (item && !results[index]?.error) {
                item.status = pendingStatusChanges[rowId];
            }
        });
...
function setupInfiniteScroll() {
    window.addEventListener('scroll', () => {
        if (
            ...
            if (displayLimit < filteredOrderGroups.length) {
